...
- 作業ディレクトリ: `D:\AI\AdobePlugin\Illustrator\artboard-batch-export-queue`
...
```powershell
cd D:\AI\AdobePlugin\Illustrator\artboard-batch-export-queue
npm test
npm start
```
...
5. Illustrator ExtendScript + ScriptUI の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/artboard-batch-export-queue-docs.zip` に存在する。
